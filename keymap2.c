...
#define RLQT A(KC_9) // «
#define RRQT A(KC_O) // »
#define LCBR A(KC_LEFT_BRACKET) // {
#define RCBR A(KC_LEFT_BRACKET) // }
#define PIPE A(KC_BACKSLASH) // |
#define LSBR A(KC_SEMICOLON) // [
#define RSBR A(KC_QUOTE) // ]
#define LESS A(KC_COMMA) // <
#define GRET A(KC_DOT) // >
#define GRAV A(KC_SLASH) // `
...
[_RAISE] =   LAYOUT(
      LESS,   KC_C,  KC_X,  KC_F,  LNGC,   TILD,   K_TB,   K_BC,     KC_Q,    CARE,    KC_M,    KC_P,    LSBR,   KC_V,  GREA,
      LQ0T,   ENGL,  KC_O,  C_DN,  C_UP,   KC_W,   K_LS,   K_SP,     KC_V,    KC_E,    KC_T,    KC_N,    RUSS,   APOS,  RQOT,
	             NUMR,  AMPD,  DOLR,   LCBR,   ATTT,   KC_J,     K_LA,    HASH,    RCBR,    KC_D,    KC_B,   KC_0, 
	                    KC_0,  KC_0,   KC_0,   KC_0,   KC_G,     K_EN,    KC_0,    GRAV,    PIPE,    KC_0
),
...
bool current_system = 0; // 0 - windows  1 - linux
...
// --right part
  ...
  CHORD(RU_M,         CR_M),
  CHORD(S(KC_SLASH), CR_T, CR_M),       //,  _B_B
